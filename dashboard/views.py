...
def insight_api(request):
    ...
    if not transactions.exists():
        return JsonResponse({"insight": "Registre algumas transações para eu analisar seu perfil."})
    ...
    if total_expense > total_income:
        insight = "Atenção: Suas despesas estão maiores que suas receitas neste período."
    elif total_expense > 0:
        pct = (total_expense / total_income) * 100 if total_income > 0 else 100
        insight = f"Você gastou {pct:.1f}% das suas receitas. Ótimo trabalho!"
    else:
        insight = "Você ainda não tem despesas registradas."
